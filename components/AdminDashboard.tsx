    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div className="flex gap-3">
          <button onClick={() => setIsBulkModalOpen(true)} className="px-4 py-2 bg-white border border-stone-300 text-stone-700 rounded-lg hover:bg-stone-50 transition-colors flex items-center">
            <i className="fa-solid fa-file-import mr-2"></i> Bulk Add
          </button>
          <button onClick={handleOpenAdd} className="px-4 py-2 bg-rumi-600 text-white rounded-lg hover:bg-rumi-700 transition-colors flex items-center shadow-md">
      ...
      <div className="relative mb-6">
        ...
        <input type="text" className="block w-full pl-10 pr-3 py-3 border border-stone-200 rounded-xl bg-white focus:outline-none focus:ring-1 focus:ring-rumi-500" placeholder="Search products..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
      </div>
                ...
                <tr key={product.id} className="hover:bg-stone-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="h-12 w-12 bg-stone-100 rounded-lg overflow-hidden"><img className="h-full w-full object-cover" src={product.image} /></div>
                      <div className="ml-4"><div className="text-sm font-semibold text-stone-900">{product.name}</div></div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap"><span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-stone-100 text-stone-600">{product.category}</span></td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-stone-900">Rs. {product.discounted_price.toLocaleString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button onClick={() => handleOpenEdit(product)} className="text-rumi-600 mr-4"><i className="fa-solid fa-edit"></i></button>
                    <button onClick={() => confirm(`Delete ${product.name}?`) && onDeleteProduct(product.id)} className="text-stone-400 hover:text-red-600"><i className="fa-solid fa-trash"></i></button>
                  </td>
                </tr>
              ))}
            </tbody>
